...
function loadCityList() {
	$.post(
		...
		function(dat) {
			...
			city_table.draw(city_data, {showRowNumber:true, height:"700px", width:"125px"});
			// preselect city
			...
			google.visualization.events.addListener(city_table, 'select', function(){
				selection = city_table.getSelection();
...
function drawXLabel(xs) {
	...
	for (i=1; i<=xs.length; i++) {
		...
		var lt = $("<span>").text(xs[i-1]).css({
			"position":"absolute", "left":startx+(i*interval)-left_gap, "top":starty+8, "font-size":"12px", "font-style":"italic",
		});
		$("#paper").append(lt);
	}
}
...
// don't know whether can be generalized later
// source: e.g. meituan
function _drawline(source, x, y, xlabels, ylabels) {
	var max_x = (xlabels.length+1) * 24;
	var min_x = 0;
	...
	for (i=0; i<x.length; ++i) {
		...
		var c = g_paper["p"].circle(xx, yy, 3).attr({"stroke":g_palette[source]});
		g_paper_plots.push(c);
	}
...
function draw(dat) {
	...
	var ylabels = constructYLabels(dat);
